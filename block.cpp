 ...
 */
Block::Block() //name space allows us to see the properties
{
    xpos = 0;
    ypos = 0;
    width = 0;
 ...
 */
Block::Block(int x, int y, int width, int height, fgcugl::Color color) // fgcugl::Color color = fgcugl::White
{
    xpos = x;
    ypos = y;
    this->width = width;
 ...
 */
bool Block::isEmpty() { return width + height == 0; }  // double == to say if those two are equal bc bool
...
/**
 * Draw a block with a border by drawing the block passed in on
 * top of a background
 *
 * Parameters:
 ...
 */
void Block::drawWithBorder(fgcugl::Color borderColor, int borderSize )  // getting an error, fixed by not assigning
{
