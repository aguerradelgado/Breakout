...
// desc:  main application file for a 2D Breakout game
// ----------------------------------------------------
#include <iostream>     // console library for debugging

#include "fgcugl.h"     // fgcu OpenGL library
#include "breakout.h"   // main app header file
#include "paddle.h"     // player paddle
#include "ball.h"       // the ball

//function prototypes
// creation
void createBricks(Block bricks[BRICK_ROWS][BRICK_COLUMNS]);  // array sin c++ always passed by reference
...
int main()
{
    ...
    // directionsbricks[BRICK_ROWS][BRICK_COLUMNS]
...
void createBricks(Block bricks[BRICK_ROWS][BRICK_COLUMNS])
{
    ...
    for (int row = 0; row < BRICK_ROWS; row++)
    {
        ...
        {
            fgcugl::Color blockColor; // initializing color variable

            // assign color to each row number
            switch(row)
            {
                case 0:
                case 1:
            ...
            x += BRICK_WIDTH;  // makes the bricks starts to the left of the last

        } // columns

        y += BRICK_HEIGHT;  // makes bricks start at the next row above the last
...
bool update(Ball& ball, Paddle& paddle, Block bricks[BRICK_ROWS][BRICK_COLUMNS], Walls walls, double lag, Direction next){
    ...
    else
    {
        ...
        // dont start checking until the ball has reached
        // the lowest brick
        if (ball.top() >= BRICKS_START_Y)
        {
            for (int row = 0; row < BRICK_ROWS; row++)
            {
                for (int column = 0; column < BRICK_COLUMNS; column++)
                {
                    ...
                    if (!bricks[row][column].isEmpty())  // call to function
                    {
                        Ball::CollisionType brickCollision = ball.collisionCheck(bricks[row][column]);
                        if (brickCollision)
                        {
                            collision |= brickCollision;
                            bricks[row][column] = Block(); // want to set to empty, so call to default constructor
                        }
                    } // not broken
